...
cars.then(function(data) {
  ...
  // Group container for bars
    ...
    // Creates a tooltip box, with background, size, border color, and other attributes
  ...
  // Draw bars
    barGroups.append("rect")
      ...
      .on("mouseover", (event, d) => { tooltip.style("opacity", 1).html(`Price: $${d.price.toLocaleString()}`) }) // Makes the tooltip visible when hovering over a bar
      .on("mousemove", (event) => { tooltip.style("left", event.pageX + 12 + "px").style("top", event.pageY - 28 + "px") }) // Updates the tooltip depending on cursor position
      .on("mouseout", () => { tooltip.style("opacity", 0) }); // Hides the tooltip when not over a bar
